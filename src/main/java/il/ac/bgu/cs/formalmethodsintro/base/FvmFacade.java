...
//				System.out.println("after " + i +  states_of_distance_i);
	...
	public <S1, S2, A, P> TransitionSystem<Pair<S1, S2>, A, P> interleave(TransitionSystem<S1, A, P> ts1,
		...
		for (TSTransition<S1, A> transition1 : ts1.getTransitions()) {
			for (S2 state2 : ts2.getStates()) {
				res.addTransition(new TSTransition<Pair<S1, S2>, A>(new Pair<S1, S2>(transition1.getFrom(), state2), // from
		...
		for (TSTransition<S2, A> transition2 : ts2.getTransitions()) {
			...
			for (S1 state1 : ts1.getStates()) {
				res.addTransition(new TSTransition<Pair<S1, S2>, A>(new Pair<S1, S2>(state1, transition2.getFrom()), // from
	...
	public <S1, S2, A, P> TransitionSystem<Pair<S1, S2>, A, P> interleave(TransitionSystem<S1, A, P> ts1,
		...
		for (TSTransition<S1, A> transition1 : ts1.getTransitions()) {
			for (TSTransition<S2, A> transition2 : ts2.getTransitions()) {
				if (transition1.getAction().equals(transition2.getAction())
						&& handShakingActions.contains(transition1.getAction())) {
					res.addTransition(new TSTransition<Pair<S1, S2>, A>(
							new Pair<S1, S2>(transition1.getFrom(), transition2.getFrom()), transition1.getAction(),
	...
	public <L1, L2, A> ProgramGraph<Pair<L1, L2>, A> interleave(ProgramGraph<L1, A> pg1, ProgramGraph<L2, A> pg2) {
		...
		for (L1 location1 : pg1.getLocations()) {
			for (L2 location2 : pg2.getLocations()) {
				...
				res.setInitial(location,
						pg1.getInitialLocations().contains(location1) && pg2.getInitialLocations().contains(location2));
		...
		for (PGTransition<L1, A> transition1 : pg1.getTransitions()) {
			for (L2 location2 : pg2.getLocations()) {
				res.addTransition(new PGTransition<Pair<L1, L2>, A>(new Pair<L1, L2>(transition1.getFrom(), location2),
						transition1.getCondition(), transition1.getAction(),
		...
		for (PGTransition<L2, A> transition2 : pg2.getTransitions()) {
			for (L1 location1 : pg1.getLocations()) {
				res.addTransition(new PGTransition<Pair<L1, L2>, A>(new Pair<L1, L2>(location1, transition2.getFrom()),
						transition2.getCondition(), transition2.getAction(),
...
//                int i = 0;
		...
		for (Pair<Map<String, Boolean>, Map<String, Boolean>> state : res.getStates()) {
			...
			for (Map<String, Boolean> action : res.getActions()) {
				...
				TSTransition<Pair<Map<String, Boolean>, Map<String, Boolean>>, Map<String, Boolean>> transition = new TSTransition(
						state, action, new Pair<Map<String, Boolean>, Map<String, Boolean>>(action, new_regs));
		...
		for (TSTransition<Pair<Map<String, Boolean>, Map<String, Boolean>>, Map<String, Boolean>> trans : res
				.getTransitions()) {
...
//L
	...
	public <L, A> TransitionSystem<Pair<L, Map<String, Object>>, A, String> transitionSystemFromProgramGraph(
		...
		for (L initial_location : pg.getInitialLocations()) {
			for (Map<String, Object> initial_assignment : initial_assignments) {
				Pair<L, Map<String, Object>> pair = new Pair<L, Map<String, Object>>(initial_location,
						initial_assignment);
				...
				for (PGTransition<L, A> transition : pg.getTransitions()) {
					if (state.getFirst().equals(transition.getFrom())) {
						if (ConditionDef.evaluate(conditionDefs, state.getSecond(), transition.getCondition())) {
							Pair<L, Map<String, Object>> new_state = new Pair(transition.getTo(),
									ActionDef.effect(actionDefs, state.getSecond(), transition.getAction()));
							TSTransition<Pair<L, Map<String, Object>>, A> ts_transition = new TSTransition(state,
									transition.getAction(), new_state);
	...
	public <L, A> TransitionSystem<Pair<List<L>, Map<String, Object>>, A, String> transitionSystemFromChannelSystem(
		...
		Set<Pair<List<L>, Map<String, Object>>> new_states;// this set is for states that was first found in a current
															// specification
		Set<ChannelSystemTransitionWrapper<L, A>> new_normal_transitions;// this set is for new transitions that was
																			// creaed a current specification
		Set<ChannelSystemTransitionWrapper<L, A>> new_queue_read_transitions;// this set is for states that was first
																				// found in a current specification
		Set<ChannelSystemTransitionWrapper<L, A>> new_queue_write_transitions;// this set is for states that was first
																				// found in a current specification
		...
		while (states_at_distance_i.size() != 0) {
			...
			for (Pair<List<L>, Map<String, Object>> state : states_at_distance_i) {
				for (int i = 0; i < cs.getProgramGraphs().size(); i++) {
					...
					for (PGTransition<L, A> transition : pg.getTransitions()) {
						if (transition.getFrom().equals(locations.get(i))
								&& ConditionDef.evaluate(conditions, eval, transition.getCondition())) {
							ChannelSystemTransitionWrapper<L, A> wrapper = new ChannelSystemTransitionWrapper<L, A>(i,
									transition, state);
							if (interleaving_act_def.isOneSidedAction_write((String) transition.getAction())) {// this
																												// action
																												// is
																												// write
																												// to
							...
							} else if (interleaving_act_def.isOneSidedAction_read((String) transition.getAction())) {// this
																														// action
																														// is
																														// read
																														// from
			...
			for (ChannelSystemTransitionWrapper wrapper_write : new_queue_write_transitions) {
				for (ChannelSystemTransitionWrapper wrapper_read : new_queue_read_transitions) {
					if (wrapper_read.isMatching(wrapper_write, interleaving_act_def)) {
						TSTransition<Pair<List<L>, Map<String, Object>>, A> new_trans = wrapper_read
								.get_transition_two_sided(wrapper_write, interleaving_act_def);
						if (new_trans == null) {
							continue;
						}
						if (!res.getStates().contains(new_trans.getTo())) {
	...
	private static boolean is_basic(NanoPromelaParser.StmtContext context) {
		return (context.assstmt() != null || context.chanwritestmt() != null || context.chanreadstmt() != null
				|| context.skipstmt() != null || context.atomicstmt() != null);
	}
	...
	private static void add_transitions_basic(Set<PGTransition<String, String>> transitions,
			NanoPromelaParser.StmtContext context) {

		transitions.add(new PGTransition<String, String>(context.getText(), "",
				context.skipstmt() != null ? "skip" : context.getText(), ""));
	}

	private static void add_transitions_complex(Set<PGTransition<String, String>> transitions,
			NanoPromelaParser.StmtContext context) {
		...
		for (PGTransition<String, String> transition : tmp) {
			if (transition.getFrom().equals(context.stmt(0).getText())) {
				String to = transition.getTo().equals("") ? context.stmt(1).getText()
						: (transition.getTo() + ";" + context.stmt(1).getText());
				transitions.add(
						new PGTransition<>(context.getText(), transition.getCondition(), transition.getAction(), to));
			}
		}

	}

	private static void add_transitions_if(Set<PGTransition<String, String>> transitions,
			NanoPromelaParser.StmtContext context) {
		...
		for (NanoPromelaParser.OptionContext option : context.ifstmt().option()) {
			...
			for (PGTransition<String, String> transition : transitions) {// transition is stmt1 ->(h:alpha) stmt1'
				if (transition.getFrom().equals(stmt_tag)) {
					...
					tmp.add(new PGTransition<>(context.getText(), condition, transition.getAction(),
							transition.getTo()));
				}
			}
		}
	...
	private static void add_transitions_do(Set<PGTransition<String, String>> transitions,
			NanoPromelaParser.StmtContext context) {
		...
		transitions.add(new PGTransition<>(context.getText(), "!(" + String.join("||", conditions) + ")", "", ""));
	...
	private static void add_transitions(Set<PGTransition<String, String>> transitions,
			Set<NanoPromelaParser.StmtContext> contexts) {
		...
		for (TSTransition<Sts, A> transitions_ts : ts.getTransitions()) {
			...
			for (Map.Entry<Saut, Map<Set<P>, Set<Saut>>> entry : aut_transitions.entrySet()) {
				...
				for (Saut state_aut : next_of_q_and_alpha) {
					...
					TSTransition<Pair<Sts, Saut>, A> transition_product = new TSTransition<Pair<Sts, Saut>, A>(
							from_product, transitions_ts.getAction(), to_product);
...
//			System.out.println("V :" + v);
	...
	public <S, A, P, Saut> boolean ReachFrom_helper(TransitionSystem<Pair<S, Saut>, A, Saut> product_ts,
			Pair<S, Saut> from, Pair<S, Saut> to, Stack<Pair<S, Saut>> path) {
	...
	public <S, A, P, Saut> List<Pair<S, Saut>> ReachFrom(TransitionSystem<Pair<S, Saut>, A, Saut> product_ts,
	...
	public <S, A, P, Saut> Set<Pair<S, Saut>> get_not_phi_states(TransitionSystem<Pair<S, Saut>, A, Saut> product_ts,
			Set<Saut> accepting) {
	...
	//	System.out.println(GraphvizPainter.toStringPainter().makeDotCode(gnba));
	...
	public <S, A, P> VerificationResult<S> verifyFairLTLFormula(TransitionSystem<S, A, P> ts, FairnessCondition<A> fc,
		...
		LTL<ComposedAtomicProposition<P, A>> final_ltl = LTL.and(ltlf, LTL.not(Convert(ltl)));// A->B = -AUB = -(A^-B)
																								// ,we want -(a->b) =
																								// (A^-B)
...
//			LTL<ComposedAtomicProposition<P,A>> not_always_eventually = LTL.not(always_eventually_first);
	...
	public <S, A, P> TransitionSystem<Pair<S, A>, A, ComposedAtomicProposition<P, A>> build_TSf(
		...
		for (TSTransition<S, A> transition : ts.getTransitions()) {
			...
			for (A action : ts.getActions()) {
				...
				TSTransition<Pair<S, A>, A> new_transition = new TSTransition<>(new_from, transition.getAction(),
						new_to);
